...
어떤 작업을 하려고 foo() 함수를 부른다 치자. 내용은 알필요없고 다음 단계로 넘어가는 시점만 알수있으면 된다. 다시 마련되면 다음 단계로 진행할수 있게끔 완료 상태를 알림 받을 방법이 있으면 좋을것 같다. 자바스크립트는 알림 자체를 하나의 이벤트로 보고 리스닝한다. foo() 의 완료 이벤트를 리스닝 함으로써 알림 요건을 재구성하는 것이다.
...
Promise.wrap 는 프라미스를 만드는 함수를 만든다. 얼핏 프라미스를 만드는 프라미스 공장처럼 느껴진다. 이런 것들을 ('프라미스' + '공장') 프라미서리 라고 부르면 어떨가 싶다. 콜백식 함수를 프라미스-인식형 함수로 감싸는 것을 Lifting 또는 Promisifying 라고 한다.
...
프라미스 p 입장에선 타임아웃이 외부 요소이므로 p는 계속 실행되는데, 이는 개발자가 의도한 작동 방식이 아닐 것이다. 조건문을 넣어볼까?
...
틀림없이 bar 는 x++ 와 console 사이에 실행될것이다. bar 가 없으면 결과도 당연히 2가 될것이다. 그런데 만일 bar 가 없는데도 x++ 와 console 사이에서 bar 가 실행될수 있다면 어떻게 될까?
...
next() 의 결과값은 *foo()가 반환한 값을 value 프로퍼티에 저장한 객체이다. 만일 중간에 반환했다면 yield 가 중간 반환 값 형태로 돌려준다.
...
제너레이터는 yield를 만나면 일단 멈추기 때문에 상태(스코프)가 그대로 유지된다. 다시말해 클로저 구문을 남발할 필요가 없다.
...
비동기 코드에서 난 에러를 동기적인 모양새로 처리할 수 있다는 것은 코드 가독성, 추론성면에서 큰 장점이 된다.
...
제너레이터가 프라미스를 yield 하고 나중에 이 프라미스가 제너레이터의 이터레이터를 제어하여 끝까지 진행하는 이런 패턴은 상당히 강력하다. 이미 눈치챈 사람도 있겠지만 ES7 에 async, await 추가 되었다.

async 는 메소드 앞에 붙이는 키워드로 프라미스를 반환한다. await 는 프라미스가 귀결될때 까지 함수를 멈추게 해주는 키워드 이다. 이는 앞서 본 run 같은 헬퍼없이 동기적 형태의 흐름 제어 코드와 프라미스를 겹합하는 하나의 구문 체계로 정리한 것이다.

### 4.4.2 제너레이터에서의 프라미스 동시성

지금까지 예시는 한 단계의 프라미스 + 제너레이터 비동기 흐름을 설명했으나 실제 비동기 코드는 보통 여러 단계로 구성된다. 신중하게 판단하지 않으면 제너레이터만 있어도 충분하다고 생각하고 결국 비최적 성능 패턴에 만족하게 된다.

서로 다른 두 데이터 소스에 데이터를 각각 요청한 후 수신한 응답을 조합해서 다시 세 번째 요청을 전송하고 받은 응답을 출력하는 코드를 작성해보자

```javascript
function *foo() {
    var r1 = yield request(url1);
    var r2 = yield request(url2);
    var r3 = yield request(url + r1 + r2);
    consolel.log(r3);
}
run(foo);
```

대략 이런 느낌으로 구현할것이다. 그러나 여기서 주의할점은 r1, r2는 동시 실행이 가능한 로직인데 실제로는 순차 실행되고 있다. 즉, r1 이 완료 된 후에 r2가 요청이 된다.

그럼 어떻게 동시에 요청을 보낼수 있을까? 가장 간단하게는 시간 독립적 형태로 상태 관리가 가능한 프라미스 본연의 능력을 이용하는것이다.
...
p1, p2 는 동시에 ajax 요청을 하고, 잇따른 yield 문 2개로 프라미스 귀결 값을 받을 때 까지 대기한다. 어떻게 흘러가든 p1, p2 동시에 실행되고 순서에 상관없이 둘 다 끝나면 r3 가 요청하게 된다.
또는 Promise.all([]) 로도 처리할수 있다.
...
function *foo() {
    ...
    var r1 = results[0];
    var r2 = results[1]; // or [r1, r2] = results
    // ...
}
```
...
코딩 스타일 문제이지만 제너레이터 내부에 얼마만큼의 프라미스 로직을 집어넣을지 신중히 판단하기 바란다. 지금까지 설명한 제너레이터로 비동기성을 나타내는 방법은 단순 + 순차적 + 동기적인 형태의 코드를 작성하면서도 가능한 한 비동기 관련 세부분은 감추고자 하는 의도가 지배적이다.
...
*foo() 가 내부적으로 bar()에게 어떤 결과를 요청한 뒤 yield로 기다리게 하면 코드가 훨신 깔끔해진다. 비동기성, 사실상 프라미스를 하나의 구현 상세로 보는 셈이다. 이런식으로 프라미스 로직을 어느 일반 함수에 감추면 제너레이터에서 그냥 호출하는 식으로 세부 로직을 추상화시켜 놓아야 더 상위 수준의 작업을 표현하기가 편하다.

기능 좋고 성능 좋은 코드를 작성하는 것도 좋지만 항상 되도록 추론성과 유지 보수성이 나은 코드를 작성하려고 노력햐야 한다.

## 4.5 제너레이터 위임

저네레이터 내부에서 일반 함수를 호출하는 식으로 구현 상세를 추상화시켜 로직을 감추는 기법이 유용한 이유를 살펴봤다. 그런데 일반 함수를 이용하면 일반 함수 규칙에 맞게끔 작동하기 때문에 제너레이터처럼 yield 에서 멈추게 할 수 없는 점이 가장 큰 단점이다.

다음과 같이 어떤 제너레이터를 다른 제너레이터에서 run() 헬퍼를 이용하여 호출하는 경우를 보면 이해가 갈것이다.

```javascript
function *foo() {
    var r2 = yield request(url1);
    var r3 = yield request(url2);
}

function *bar() {
    var r1 = yield request(url3);
...
bar() 를 통해 run() 유틸리티로 foo()를 실행하고 있다. 이 제너레이터의 실행이 완료되면 귀결된 프라미스를 run()이 반환할 테니 run() 인스턴스를 통해 다른 run()에서 나온 프라미스를 yield해서 내보내면 자동으로 foo()가 끝날 때 까지 bar()를 멈추게 할 거란 계산이다.

그러나 foo() 호출을 bar() 안으로 합하고자 한다면 yield 위임이라는 더 좋은 방법이 있다. 간단한 예제를 통해 알아보자
...
코드를 보면 yield * 는 이너레이터 인스턴스에 관한 제어권을 또 다른 이터레이터 foo()에게 위임한다.
그래서 두 번째 it.next() 호출까지는 bar()를 제어하지만 세번째 호출은 foo()를 시작하고 bar() 대신 foo()를 제어한다. 즉, bar()가 자신의 순회 권한을 foo() 에게 위임했다고 볼 수 있기 때문에 위임이라는 말이 붙은 것이다.

it 이터레이터로 전체 foo() 이터레이터를 훑고나면 자동으로 제어권은 bar()로 넘오온다. 앞의 예제에 위임을 적용해보면

```javascript
function *foo() {
    var r1 = yield request(url1);
    var r2 = yield request(url2 + r1);
    return r2;
}
function *bar() {
    var r1 = yield request(url1);
...
여기서 yield * 가 내어주는거는 이터레이터의 제어권이다. 이는 모든 이터러블 에 가능하다.

### 4.5.1 왜 위임을

가장 큰 목적은 일반 함수 호출과 맞추기 위함이다. 일반 함수 처럼 별개의 함수로 나누어 프로그램 가독성, 유지 보수성. 디버깅 측면에 유리하도록 한것이다.

### 4.5.2 메세지 위임

yield 위임은 이터레이터뿐 아니라 양방향 메시징에도 쓰인다. 다음 코드를 보자

```javascript
function *foo() {
    console.log('*foo() in ', yield 'b');
    console.log('*foo() in ', yield 'c');
    return 'd';
}
function *bar() {
    console.log('*bar() in ', yield 'a');
    console.log('*bar() in ', yield *foo());
    console.log('*bar() in ', yield 'e');
    return 'f';
}
var it = bar();
console.log('out ', it.next().value);
// out: a
console.log('out ', it.next(1).value);
// *bar() in 1
// out b
console.log('out ', it.next(2).value);
// *foo() in 2
// out c
console.log('out ', it.next(3).value);
// *foo() in 3
// *bar() in d
// out e
console.log('out ', it.next(4).value);
// *bar() in 4
// out f
```

it.next(3) 호출 이후 처리 단계에 집중해보자

1. 대기 중인 foo()의 yield 'c' 표현식으로 3이 전해진다.
2. foo() 에서 return 'd' 해도 이 값이 foo() 외부로 빠져나와 it.next(3) 호출부에 그대로 돌아오진 않는다.
3. 대신 'd' 는 bar() 에서 기다리고 있는 yield *foo() 표현식의 결과값으로 반환된다. 즉, yield 위임 표현식은 모든 foo() 의 실행이 끝날 때 까지 마냥 기다린다. 그래서 bar() 내부에선 'd' 가 출력할 최종값이다.
4. bar() 내부에서 yield 'e'를 호출하고 'e'는 it.next(3) 호출의 결과값으로 yield되어 나온다.

외부 이터레이터 입장에서는 원래 제너레이터나 위임 제너레이터나 전혀 다른 점이 없다. yield 위임은 앞서 말했듯이 이터러블에도 쓸수 있다.
...
이때 이터러블 배열의 경우 next() 호출을 거쳐 보내진 메시지에는 전혀 관심이 없어서 무시된다. 또한 명시적인 반환 값이 없으므로 undefined를 받는다.
...
console.log('out ', it.next(1).value);
// out b
console.log('out ', it.throw(2).value);
// *foo err : 2
// out c
console.log('out ', it.next(3).value);
// *bar err : d
...
2. 그리고 foo() -> bar() 방향으로 전파되어 던져진 'd' 는 bar()가 잡아 처리한다. 그런 다음 yield 'e' 는 it.next(3) 호출의 결과값 'e' 를 보낸다.
3. 다음 baz() 에서 발생한 예외는 bar() 에서 잡히지 않는다. 따라서 baz(), bar() 모두 완료가 된다.

### 4.5.3 비동기성을 위임

### 4.5.4 위임 '제귀'

비동기형 제너레이터의 재귀, 즉 스스로에게 yield 위임하는 제너레이터 작성이 가능하다.
...
4. 2 > 1 이므로 foo(1) 역시 다른 이터레이터를 생성후 val에 1을 넘긴다.
5. 1 > 1 은 false 이므로 request() 를 val 1로 호출하고 해당 프라미스를 반환한다.
6. 이 프라미스가 yield 되어 나오면 foo(2) 제너레이터 인스턴스로 돌아간다.
7. yield *는 이 프라미스를 다시 foo(3) 제너레이터 인스턴스로 보낸다. 다시 이는 bar() 제너레이터 인스턴스로 보내고 다시 run 유틸리티로 보내어 해당 프라미스가 귀결되기를 기다린다.
8. 프라미스가 귀결되면 bar() 를 재게하고 foo(3) -> foo(2) 제너레이터 인스턴스로 전해진다.
9. foo(2) 인스턴스 결과를 다시 request 요청을 한다
10. 반복

## 4.6 제너레이터 동시성

상일란 2개의 동시 ajax 응답 처리기를 데이터 통신 과정에서 경합 조건이 발생하지 않게끔 응답을 res 배열에 넣었던 코드를 제너레이터로 구현해보자
...
수동으로 res[0], res[1] 할당할 필요 없이 res.push() 가 적절히 기대/의도한 순서대로 값을 넣도록 조정해서 조금 더 깔끔한 느낌이 든다. 하지만 이런 상호 작용을 어떻게 맞출까? 먼저 수동으로 해보자
...
두 reqDaa() 인스턴스가 ajax 요청 후 yield 에서 멈춘다. 그 후 p1이 귀결되면 첫번째 인스턴스를 재개하고 그다음 p2가 귀결되면 두번째 인스턴스를 재개한다. 하지만 이는 손이 많이 가고 실제로 제너레이터 자체를 조정하는게 아니라 효율적이지 못하다. 다른 방법을 살펴보자
...
여전히 수동적이지만 이전 예제는 첫 번째 인스턴스가 완전히 끝난 다음에야 두 번째 인스턴스에 데이터를 넘겼었다. 하지만 이제 두 인스턴스 모두 각자의 응답이 돌아아조마자 데이터를 수신하고 각 인스턴스는 제어권을 넘길 요량으로 한번 더 yield 한다. 그 후 Promise.all 처리기에서 어떤 순서로 재개할지 결정한다.

아직 더 개선할 여지가 많다. 다음 runAll() 이란 유틸리티를 생각해보자
...
첫 번째 제너레이터는 요청에 대한 응답 프라미스를 얻고 runAll() 유틸리티로 제어권을 넘겨준다.
두 번째 제너레이터 역시 요청에 대한 응답 프라미스를 얻고 runAll() 유틸리티로 제어권을 넘겨준다.

1. 첫 번째 제너레이터 재개 후 자신의 프라미스 p1를 yield한다. 이 때 runAll() 유틸리티가 하는 일은 프라미스 귀결을 기다렸다가 같은 제너레이터를 재개한다는 점에서 앞의 run() 과 동일하다. p1 이 귀결되면 runAll()은 첫 번째 제너레이터를 귀결 값으로 재개 후 res[0]에 해당 값을 할당한다. 첫 번째 제너레이터가 끝나면 암시적으로 제어권 넘김이 이루어진다.
2. 두 번째 제너레이터가 재개되고 p2 프라미스를 yield한 후 귀결될 때까지 대기한다. 귀결되면 runAll() 은 이 값으로 두 번째 제너레이터를 재개하고 res[1]을 세팅한다.

runAll()을 확장하여 다중 제너레이터 인스턴스가 공유할 수 있는 내부 변수 공간을 제공하면 더 유용할것이다.
...
지금까지 제너레이터에서 프라미스를 yield하고 run() 같은 헬퍼를 통해 이 프라미스가 제너레이터를 다시 시작하게 만드는 방법이 제너레이터로 비동기성을 관리하는 최선이라는 전제하에 설명을 이어왔다. 그런데 다른 패턴도 존재한다.

일반 컴퓨터 과학에 썽크라는 개념인데. 다른 함수를 호출할 운명을 가진 인자가 없는 함수같은 것이다. 즉, 어떤 함수 정의부를 또 다른 함수 호출부로 감싸 실행을 지연시키는데 여기서 감싼 함수가 바로 씽크다.
예를 들면
...
보다시피 동기적 싱크는 직관적이다. 다음은 비동기 씽크 예시이다.
...
인자가 이미 지정되어 fooThunk() 는 콜백 함수만 넣어주면 된다. 그러나 일일이 수동으로 썽크를 작성하는것보다는 유틸리티를 작성하는것이 좋아보인다.
...
코드에서 thunkiy() 는 foo() 함수의 레퍼런스와 필요한 인자를 입력받고 썽크 자신을 도로 반환한다. 하지만 자바스크립트에서는 보통 썽크 자신을 생성하기 보다는 썽크를 만드는 함수를 생성한다.
...
whaiIsThis 는 썽크를 생산하는 공장(factory)같은것이다. 하지만 이는 쓸데없이 복잡해 보일수도 있지만, 썽크가 필요한 시점에서만 호출해서 깔끔하게 분리시킬수 있다.
...
그런데 이게 제너레이터와 무슨 상관일까?

작동 개념이 동등하지 않아서 직접적인 상호 호환성은 없다. 사실만 보면 프라미스가 더 능력이 좋다. 하지만 다른 의미로 둘 다 어떤 값을 요청하여 비동기적 응답을 받는다는 점은 같다.

이전 내용에서 함수를 프라미스화 하는 Promise.wrap() 라는 유틸리티를 소개했었는데 이 역시 같은 맥락이다. 이 역시 직접 프라미스를 생성하는 대신 나중에 프라미스를 생성할 프라미서리를 만들어낸다.

thunkify() 와 promiseify() 둘다 기본적으로 어떤 값을 질의하고 이 질문에 대한 미랫값을 나타내는 대칭 관계이다. 이런 관점에서 제너레이터는 비동기성 프라미스 대신 비동기성 썽크를 yield 해도 된다.

하지만 더 넓게보면 썽크 자체는 프라미스의 믿음성/조합성이 보장하지 못한다. 제너레이터의 비동기 패턴에서 프라미스 대용으로 쓸 수는 있지만 이상적인 해결책은 아니다.

## 4.8 ES6 이전 제너레이터

제너레이터는 ES6 이후에 나온 신생 구문이므로 ES5 이하 환경에서는 트랜스파일러를 이용해야한다.

### 4.8.1 수동 변환

먼저 제너레이터를 수동으로 트랜스파일 하는 방법을 보자.
...
알단 호출 가능한 일반 함수 foo()가 필요하고 이 함수는 이터레이터를 반환해야함을 알 수 있다. 그럼 대략 이런 윤곽이 잡힐것이다
...
제너레이터는 자신의 스코프/상태를 지연시키는 마술사로 함수 클로저로도 비슷하게 흉내낼 수 있다. 먼저 상태값을 기준으로 제너레이터 코드를 각각 나누어 살펴보자
...
이 코드는 상태 1 에서 시작하여 요청이 성공하면 상태 2, 실패하면 상태 3으로 넘어간다. 이런식으로 얼마든지 yield 단계를 늘릴수 있다.

다시 트랜스파일된 제너레이터로 돌아가보자. 먼저 클래저에서 상태 추적용 변수를 정의하고, 조건문을 써서 상태에 따라 처리하는 내부 함수를 정의하자.
...
제너레이터의 모든 상태는 이 switch 문의 case로 나타내고 상태가 바뀔 때마다 process()를 호출한다. process() 를 여러 번 호출해도 제너레이터 스코프의 변수값이 유지되도록 process() 바깥쪽 선언부로 위치를 옮긴다. err 의 경우만 상태 3에서만 필요하므로 놔둔다.

상태 1에서 yield resolve() 대신 return resolve() 했고, 상태 2 끝에 명시적 return 이 없으므로 그냥 return 한다. 상태 3 끝에는 return false 를 그대로 둔다.
...
1. 이터레이터의 next()를 처음 호출하면 제너레이터는 초가화하지 않은 상태에서 상태 1로 변경되고 process() 를 호출하여 상태 1을 처리한다. request() 반환값인 프라미스는 next() 의 value 로 돌려준다.
2. 프라미스가 이룸되면 next() 를 두 번째 호출하여 응답값을 보내고 상태 2가 된다. 이 인자로 process() 를 재호출하고 next() 호출의 value 프로퍼티는 undefined가 된다.
3. 프라미스가 실패하면 throw() 가 호출되고 상태 1에서 3으로 변경된다.

### 4.8.2 자동 변환

변환 과정이 yield 가 늘어날수록 상당히 복잡해진다. 뿐만 아니라 같은 코드를 다른 제너레이터에 이식하기도 상당히 힘들다. 무엇보다도 일일이 수동 변환한다는것 자체가 비현실적이다. 다행이 자동 변환 툴이 존재하는데 이는 페이스북 개발자들이 만든 regenerator 라는 툴이 있다.
